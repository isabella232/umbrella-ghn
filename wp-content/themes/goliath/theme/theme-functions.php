    ...
    function plsh_is_post_hot($post_id)
    {
        ...
        if(class_exists('WordpressPopularPosts'))
        {
            ...
            if(empty($cached) || (!empty($cached) && $cached['timestamp'] < time()))  //if cache is older one hour
            {
                ...
                $interval = "1 WEEK";
                $now = current_time('mysql');
                $query = "SELECT pop.postid FROM {$table_name}summary as pop, {$wpdb->prefix}posts as p WHERE pop.postid = p.ID AND pop.view_datetime > DATE_SUB('{$now}', INTERVAL {$interval}) AND p.post_type = \"post\" GROUP BY pop.postid ORDER BY SUM(pop.pageviews) DESC LIMIT 5";
