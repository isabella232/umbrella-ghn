...
<div class="container homepage-content hfeed thumb-<?php echo esc_attr($post_image_width); ?>">
    ...
    <div class="main-content-column-1 <?php if(plsh_gs('sidebar_position') === 'left') { echo ' right'; } ?>">
        ...
        <div <?php post_class($class); ?> itemscope itemtype="http://data-vocabulary.org/<?php if(plsh_is_review()) { echo 'Review'; } else { echo 'NewsArticle'; } ?>">
            ...
            <div class="post" 
                <?php if(plsh_is_review()) { echo ' itemprop="description"'; } else { echo ' itemprop="articleBody"'; } ?>><?php the_content();
                
                // Add Special fields for GHN on Air post type //////////////////////////
                $audio_url   = get_field( "audio_url" );
                $source_name = get_field( "credits_source_name" );
                $source_url  = get_field( "source_url" );

                //Print source name if provided and add link if provided
                if ($source_name) {
                ?>
                <p>
                    <strong>Source:</strong> 
                    <?php if($source_url){ ?>
                    <a href="<?echo $source_url ; ?>" target="_blank">
                ...
                // Print download button if audio URL provided 
                if( $audio_url ) {
                ?>
                <p>
                    <a class="btn btn-default" href="<?php echo $audio_url; ?>" target="_blank" rel="noopener noreferrer">Listen Online</a> 
                </p>
                <?php
                }                
                ?>
